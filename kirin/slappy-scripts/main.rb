...
hear MYNAME do |event|
  if event.text.include?("今日")
    res = AnimePG.syobocal_today_midnight()
    say "先輩、御用でしょうか。今日の深夜アニメの放送予定は、", channel: event.channel
    res.each{|prog|
      say AnimePG.make_title(prog, false), channel: event.channel
    }
    ...
    if key.nil? || key == ''
      ...
      ans = AnimePG.syobocal_search(key)
      if ans[0].empty? && ans[1].empty?
        say "ふああ、ごめんなさいです、先輩。「#{key}」という作品は見つけられませんでした。はうぅ…", channel: event.channel
      else
        if !(ans[0].empty?)
          say "地上波では、", channel: event.channel
          ans[0].each{|prog|
            say AnimePG.make_title(prog, true), channel: event.channel
          }
        end
        if !(ans[1].empty?)
...
schedule '45 21 * * *' do
  res = AnimePG.syobocal_today_midnight()
  say "先輩、こんばんは。今日の深夜アニメの放送予定は、", channel: '#anime_pg'
  res.each{|prog|
    say AnimePG.make_title(prog, false), channel: '#anime_pg'
  }
  say "です。夜更かししないでください…です。", channel: '#anime_pg'
end

schedule '45 6 * * 0,6' do
  res = AnimePG.syobocal_today_morning()
  say "おはようございます、先輩。今朝のアニメの放送予定は、", channel: '#anime_pg'
  res.each{|prog|
    say AnimePG.make_title(prog, false), channel: '#anime_pg'
  }
  say "です。休日も早起きしなきゃです…はぅぅ…", channel: '#anime_pg'
end

schedule '45 16 * * *' do
  res = AnimePG.syobocal_today_evening()
  say "先輩、今晩のアニメの放送予定は、", channel: '#anime_pg'
  res.each{|prog|
    say AnimePG.make_title(prog, false), channel: '#anime_pg'
  }
  say "です。今日もあと少し、ファイトです！", channel: '#anime_pg'
end
